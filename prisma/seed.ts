 ...
 * - 8 blog articles (articles NOT stored in DB — they remain as static MDX;
 *   this script only seeds catalog + coupons)
 * - 2 coupons
 * - 1 admin user
 *
 * RUN:
...
// In Phase 2, Better Auth will create the admin user via the signup flow.
// We'll then promote them to ADMIN role via a script.
// ----------------------------------------------------------------------------
...
async function main() {
  ...
  console.log("  2. Visit http://localhost:3000 to verify products load");
  console.log(
    "  3. Phase 2 will add auth — admin user will be created via signup flow.",
  );
}

...
import { signup as signupAction, login as loginAction, signInWithOAuth } from '@/lib/auth/actions';
...
function AuthForm({ modeProp }: { modeProp: 'login' | 'signup' }) {
  ...
  const titleRef = useTextReveal<HTMLHeadingElement>({
    duration: 0.7, stagger: 0.05, start: 'top 90%',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      if (!agreeTerms) {
        toast({ title: 'Please accept terms', description: 'You must agree to the Terms of Service and Privacy Policy.', variant: 'destructive' });
        return;
      }
    }
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Could not initiate OAuth login.', variant: 'destructive' });
      setIsSubmitting(false);
    }
  };
    ...
    <div className="w-full min-h-screen flex">
      <div className="hidden lg:flex w-[45%] xl:w-[50%] relative overflow-hidden flex-col items-center justify-center aura-surface-dark">
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-12">
          ...
          <p className="text-[var(--color-gold-soft)] text-sm mt-6 max-w-sm leading-relaxed" data-reveal="up" data-reveal-delay="200">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 sm:px-8 lg:px-16 pt-24 sm:pt-28 pb-8">
        ...
        {signupSuccess ? (
          <div className="w-full max-w-md text-center rounded-2xl p-8 sm:p-10 aura-surface-card aura-shadow-md">
            <CheckCircle2 className="w-16 h-16 mx-auto mb-4 aura-text-gold" />
            <h1 className="aura-h2 aura-text-primary mb-2">Check Your Email</h1>
            <p className="aura-body aura-text-secondary mb-6">
              We&apos;ve sent a verification link to <strong>{signupEmail}</strong>.
              Click the link to activate your account.
            </p>
            <Link href="/auth/login" className="aura-btn aura-btn-secondary">
              Back to Sign In
            </Link>
          </div>
        ) : (
        <div className="w-full max-w-md rounded-2xl p-8 sm:p-10 aura-surface-card aura-shadow-md">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {mode === 'signup' && (
              <Input label="Full Name" type="text" placeholder="Your full name" value={signupName} onChange={(e) => setSignupName(e.target.value)} leftAdornment={<User className="w-4 h-4" />} autoComplete="name" />
            )}
            ...
            {mode === 'signup' && (
              <Input label="Confirm Password" type={showConfirmPassword ? 'text' : 'password'} placeholder="Re-enter your password" value={signupConfirmPassword} onChange={(e) => setSignupConfirmPassword(e.target.value)} leftAdornment={<Lock className="w-4 h-4" />} rightAdornment={<PasswordToggle shown={showConfirmPassword} onToggle={() => setShowConfirmPassword((prev) => !prev)} />} autoComplete="new-password" />
            )}
            {mode === 'login' && (
              <div className="flex items-center justify-between">
                <Checkbox label="Remember me" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} />
                <Link href="/auth/forgot-password" className="text-xs font-medium transition-colors duration-200 hover:text-[var(--color-gold-hover)] cursor-pointer">Forgot Password?</Link>
              </div>
            )}
            {mode === 'signup' && (
              <Checkbox label={<>{'I agree to the '}<Link href="/terms" className="aura-text-gold font-medium cursor-pointer hover:underline">Terms of Service</Link>{' and '}<Link href="/privacy" className="aura-text-gold font-medium cursor-pointer hover:underline">Privacy Policy</Link></>} checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} />
            )}
...
export default function AuthView({ mode = 'login' }: { mode?: 'login' | 'signup' }) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin w-8 h-8 border-2 border-[var(--color-gold)] border-t-transparent rounded-full" /></div>}>
      <AuthForm modeProp={mode} />
    </Suspense>
  );
}
